...
def procesar_respuesta(datos,resp_usuario,turno):
    ...
    if resp_usuario==datos[PALABRA][turno][0]:
        datos[ACIERTOS]+=1
        datos[RESULTADOS][turno]=[CORRECTO] 
        datos[RESPUESTA][turno]=resp_usuario
    else:
        datos[ERRORES]+=1
        datos[RESULTADOS][turno]=[INCORRECTO] 
        datos[RESPUESTA][turno]=resp_usuario
    return datos
...
def seleccionar_palabras(diccionario, letras):
    ...
    Post: Devuelve un diccionario ordenada alfabéticamente con palabras selectas al azar del diccionario
